...
var LEVEL_TIME:TimeInterval = 1 * 60
...
class CalculateController: UIViewController {
    ...
    var complexity: Complexity = Complexity.square
    let maxDigit: Int = 9
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let highScores = UserDefaults.standard.value(forKey: self.complexity.rule()) as? String {
            self.highScoresLabel.text = NSString(format: "%@: %@", localized("High Score"), highScores) as String
        }  else {
            UserDefaults.standard.setValue("", forKey: self.complexity.rule())
        }
    }
    ...
    func reportScore(userScore: Int) {
        if let highScores = UserDefaults.standard.value(forKey: self.complexity.rule()) as? String {
            if  (userScore > (Int(highScores) ?? 0)) {
                ...
                UserDefaults.standard.setValue(scoresStr, forKey: self.complexity.rule())
            }
        }
    }
    ...
    func over() {
        ...
        gameOverView.text = NSString(format:"%@ %@", localized("Игра закончена!\nВаш результат"), scoresStr) as String;
    ...
    @objc func levelTick () {
        ...
        levelLabel.text = NSString(format:"%@: %d", localized("Уровень"), level) as String;
        if level < MAX_LEVEL {
            levelTickStarted = NSDate().timeIntervalSince1970
            levelsTimer = Timer.scheduledTimer(timeInterval: LEVEL_TIME, target: self, selector: #selector(levelTick), userInfo: nil, repeats: false);
        } else {
    ...
    @objc func tick() {
        if (currentDigitView == nil) {
            if  falledViews.count > 0 {
                for view in falledViews {
                    ...
                    let removes = self.check(view: dView)
                    if  (removes.count > 0) {
                        break
                    }
                }
        ...
        if let digitView  = row[xPos] as? DigitView {
            ...
            if digitView.digit > 0  {
                ...
                let _ = self.check(view: self.currentDigitView)
                currentDigitView = nil;
                return;
            }
        }
        self.playVolume(path: "down")
    ...
    func check(view: DigitView?) -> NSArray {
    ...
    func removeViews(views : NSArray) -> NSMutableArray {
        ...
        self.animateViews(views: views);
        scoresLabel.text = NSString(format:"%@: %d", localized("Scores"), scores) as String;
        return allAboveViews;
    }
    
    func animateViews(views: NSArray) {
